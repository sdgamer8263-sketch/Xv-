...
const StartupContainer = () => {
    ...
    return !data ? (
        ...
        <ServerContentBlock title={t('startup-settings')} showFlashKey={'startup:image'} icon={AdjustmentsIcon}>
            <div css={tw`md:flex gap-4`}>
                ...
                <TitledGreyBox title={t('docker-image')} css={tw`flex-1 lg:flex-none lg:w-1/3`}>
                    {Object.keys(data.dockerImages).length > 1 && !isCustomImage ? (
                        <>
                            ...
                            <p css={tw`text-xs text-neutral-300 mt-2`}>{t('select-docker-feature')}</p>
                        </>
                    ) : (
                        <>
                            <Input disabled readOnly value={variables.dockerImage} />
                            {isCustomImage && (
                                <p css={tw`text-xs text-neutral-300 mt-2`}>{t('custom-docker-image')}</p>
                            )}
                        </>
                    )}
                </TitledGreyBox>
            </div>
            <h3 css={tw`mt-6 mb-2 text-2xl`}>{t('variables')}</h3>
